...
describe("Testing validation and compilation by script's labels and operands", () => {
    ...
    describe("Testing valid code without labels but has operand", () => {
        ...
        describe("Invalid", () => {
            ...
            const errorPart1 = "Out-of-bounds address error at the operand of line 1 - label ";
            const errorPart2 = " is integer but out of bounds (0-99)";
...
describe("Testing validation and compilation by script's opcodes", () => {
    ...
    describe("opcodes that require a corrosponding operand", () => {
        ...
        describe("each one invalid", () => {
            const errorExpectedOperandLine0 = "operand expected error at the operand of line 0 - operand expected for opcode but got nothing";
    ...
    describe("opcodes that require no corrosponding operand", () => {
        ...
        describe("each one invalid", () => {
            const errorUnexpectedOperandLine0 = 'non-alphanumeric error at the operand of line 0 - token "invalidOperand" is not alphanumeric. Alphanumeric must only comprise of numbers digits (0-9) and letters (A-Z)';
            test("shift left (SHL)", () => {
                const preCompiled = [["", "SHL", "invalidOperand"]];
                const compiler = new Compiler();
                const compiled = compiler.validateAndCompile(preCompiled);
                expect(compiled).toStrictEqual([-1]);
            ...
            test("shift right (SHR)", () => {
                const preCompiled = [["", "SHR", "invalidOperand"]];
                const compiler = new Compiler();
                const compiled = compiler.validateAndCompile(preCompiled);
                expect(compiled).toStrictEqual([-1]);
                expect(compiler.getMessage()).toStrictEqual(errorUnexpectedOperandLine0);
            ...
            test("input (INP)", () => {
                const preCompiled = [["", "INP", "invalidOperand"]];
                const compiler = new Compiler();
                const compiled = compiler.validateAndCompile(preCompiled);
                expect(compiled).toStrictEqual([-1]);
                expect(compiler.getMessage()).toStrictEqual(errorUnexpectedOperandLine0);
            ...
            test("output (OUT)", () => {
                const preCompiled = [["", "OUT", "invalidOperand"]];
                const compiler = new Compiler();
                const compiled = compiler.validateAndCompile(preCompiled);
                expect(compiled).toStrictEqual([-1]);
                expect(compiler.getMessage()).toStrictEqual(errorUnexpectedOperandLine0);
            ...
            test("output as ASCII character (OTC)", () => {
                const preCompiled = [["", "OTC", "invalidOperand"]];
                const compiler = new Compiler();
                const compiled = compiler.validateAndCompile(preCompiled);
                expect(compiled).toStrictEqual([-1]);
                expect(compiler.getMessage()).toStrictEqual(errorUnexpectedOperandLine0);
            ...
            test("halt (HLT)", () => {
                const preCompiled = [["", "HLT", "invalidOperand"]];
                const compiler = new Compiler();
                const compiled = compiler.validateAndCompile(preCompiled);
                expect(compiled).toStrictEqual([-1]);
                expect(compiler.getMessage()).toStrictEqual(errorUnexpectedOperandLine0);
        ...
        describe("Specifically the Data Location opcode (DAT)", () => {
            ...
            describe("Valids", () => {
                ...
                test("just 'DAT' and a label", () => {
                    const preCompiled = [["LABEL", "DAT", ""]];
                    const compiler = new Compiler();
                    const compiled = compiler.validateAndCompile(preCompiled);
                    expect(compiled).toStrictEqual([0]);
                });
                test("just 'DAT' and a valid numerical operand", () => {
                    const preCompiled = [["", "DAT", "68"]];
                    const compiler = new Compiler();
                    const compiled = compiler.validateAndCompile(preCompiled);
                ...
                test("'DAT' with a label and valid numerical operand.", () => {
                    const preCompiled = [["LABEL", "DAT", "68"]];
                    const compiler = new Compiler();
                    const compiled = compiler.validateAndCompile(preCompiled);
                    expect(compiled).toStrictEqual([68]);
                });
                test("using DAT to store the lowest possible integer", () => {
                    const preCompiled = [["", "DAT", "-999"]];
                    const compiler = new Compiler();
                    const compiled = compiler.validateAndCompile(preCompiled);
