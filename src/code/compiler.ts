enum ErrorType{ labelDuplicate, labelInvalid, labelMissing, instructionInvalid, addressinvalid, addressOutOfBounds, addressOutOfBoundsDAT, addressExpected, addressNotExpected, opcodeAllAbsent, tokenNotAlphanumeric }
...
export class Compiler{
    ...
    private generateErrorMessage(lineId:number, tokenType:TokenType, errorType:ErrorType, invalidToken:string):string{
        ...
        const message:string = ` error at the ${TokenType[tokenType]} of line ${lineId} - `
    ...
    private errorMessage(lineId:number, tokenType:TokenType, errorType:ErrorType, invalidToken:string):void{
    ...
    private labelMapping(script:string[][]):Map<string, number>{
        ...
        for (let lineId = 0; lineId < script.length; lineId++){
            ...
            if (labels.has(token)){
                ...
                return new Map<string, number>([["", -1]]);
    ...
    public validateAndCompile(script:string[][]):number[]{
        ...
        const instructions:string[] = ["ADD", "SUB", "STA", "SHL", "SHR", "LDA", "BRA", "BRZ", "BRP", "INP", "OUT", "OTC", "HLT", "DAT"];
        //^ Does not include "DAT" as it is an opcode but not necessarily an instruction.
        const noOperands:string[] = ["SHL", "SHR", "INP", "OUT", "OTC", "HLT"];
        //^ Instructions that where either operands are forbidden.
        ///const mandatoryOperands:string[] = ["ADD", "SUB", "STA", "LDA", "BRA", "BRZ", "BRP", "DAT"];
        const compiledInstructions:Map<string, number> = new Map([["ADD",1],["SUB",2],["STA",3],["SHL",401],["SHR",402],["LDA",5],["BRA",6],["BRZ",7],["BRP",8],["INP",901],["OUT",902],["OTC",903],["HLT",0]])
        ...
        const labels:Map<string, number> = this.labelMapping(script);
        if (labels.get("") == -1) { return [-1]; }
        //^ no error sending here because it happens in the label mapping private method ('labelMapping')

        let compiledInstruction:string = "";
                ...
                switch (column){
                    ...
                    default: //< TokenType.operand
                        ...
                        if (opcodeDAT){
                            ...
                            if (!(/^-?\d{1,3}$/).test(token)){ this.errorMessage(lineId, column, ErrorType.addressOutOfBoundsDAT, token); return [-1]; }
                        ...
                        if (!(/^\d{1,2}$/).test(token)){ this.errorMessage(lineId, column, ErrorType.addressOutOfBounds, token); return [-1]; }
