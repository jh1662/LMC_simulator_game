...
export class URLQuery{
    ...
    private parseAndValidate(data:string[]):string{
        ...
        for (let config of data){
            ...
            if (!Number.isInteger(this.parsedData.at(-1)) || Number.isNaN(this.parsedData.at(-1))){ return `${config} is not a valid argument - must be an integer` }
            //^ simpler to use "data.at(-1)" than "data[data.length-1]"
            if (this.parsedData.at(-1) as number < 0){ return `${config} is not a valid argument - must be a non-negative integer` }
        ...
        if (this.parsedData[0] as number > 1){ return 'invalid argument option - sound effects config can only be 1 or 0'}
        ...
        if (this.parsedData[3] as number > 20 || this.parsedData[3] as number == 0){ return 'invalid argument option - current campain level can only be from 1 to 20'}
        return "";
